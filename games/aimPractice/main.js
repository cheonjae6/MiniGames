...
function mainmenuscreen() {
  ...
  if (
    mousePos.x > width / 2 - 100 &&
    mousePos.x < width / 2 + 100 &&
    mousePos.y > height / 2 + 50 &&
    mousePos.y < height / 2 + 100
  ) {
    ctx.drawImage(
      startbutton_lightImg,
      width / 2 - 100,
      height / 2 + 50
    );
    console.log;
...
function update() {
  if (mainmenu) {
    ...
    if (timeleft >= 0) {
      ...
      target.update();
      timeleft =
        startTime - Math.round(new Date().getTime() / 1000) + 30;
      render();
    } else {
      gameoverscreen();
    }
